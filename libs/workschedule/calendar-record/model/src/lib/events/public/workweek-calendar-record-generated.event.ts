import { CountryCode, ISOFormatDate } from '@payfit/common-time-model'
import {
  EdpBaseEvent,
  EdpBasePayloadEvent,
  EdpSchemaChildType,
...
export interface IWorkweekCalendarRecordGeneratedPayload {
  country: CountryCode
  jlCompanyId: string
  jlContractId: string
...
@EventProducerSchema({
  eventType:
    WorkscheduleCalendarRecordPublicEventEnum.WORKWEEK_CALENDAR_RECORD_GENERATED,
  subjectType: 'jlContractId',
  description: 'Workweek Calendar Record Generated Event',
  majorVersion: 6,
  schemaName: 'workweek-calendar-record-generated',
})
export class WorkweekCalendarRecordGeneratedPayload
  extends EdpBasePayloadEvent
  implements IWorkweekCalendarRecordGeneratedPayload
{
  @IsString()
  @IsDefined()
  ...
  constructor(payload: IWorkweekCalendarRecordGeneratedPayload) {
    super()
...
export class WorkweekCalendarRecordGeneratedEvent extends EdpBaseEvent<IWorkweekCalendarRecordGeneratedPayload> {
  constructor(
    subjectId: string,
    eventEffectiveTime: number,
    payload: IWorkweekCalendarRecordGeneratedPayload,
  ) {
    super({
      subjectId,
      subjectType: 'jlContractId',
