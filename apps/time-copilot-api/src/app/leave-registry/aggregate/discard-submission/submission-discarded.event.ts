...
export interface ISubmissionDiscardedPayload {
  submissionId: string;
...
@EventProducerSchema({
  ...
  description: 'Submission discarded',
  majorVersion: 2,
  schemaName: 'submission-discarded',
})
export class SubmissionDiscardedPayload extends EdpBasePayloadEvent {
  @IsUUID()
  @IsDefined()
  ...
  @JSONSchema({
    description:
      'The actor who discarded the submission (id, name & role fields)',
  })
  public actor: Actor;

  constructor(payload: ISubmissionDiscardedPayload) {
    super();
...
export class SubmissionDiscardedEvent extends EdpBaseEvent<SubmissionDiscardedPayload> {
  constructor(subjectId: string, payload: SubmissionDiscardedPayload) {
    super({
      subjectId,
